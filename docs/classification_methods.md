...
当前仓库内置的是 synthetic demo data。它的正值自然断点结果是 `1-165`、`166-315`、`316-540`、`541-760`、`761-1280`，再加上单独的 `0` 类。这个结果是由 CSV 数据自动计算得到的，不是硬编码断点。
...
当前演示数据是人为构造的非真实数据，数值范围覆盖低值、中值和高值，以便展示自然断点分级在专题图中的效果。如果使用等距分级，部分等级可能仍然较空；自然断点法会根据当前 CSV 中的数值聚类自动调整断点，更适合演示“数据驱动分级”的工作流。
